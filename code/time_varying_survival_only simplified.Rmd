...
do_internal_cv_time_varying_cox <- function(data, folds = 3, iterations = 10, shorten_visits = FALSE, n_visits = NULL) {
  ...
  for (iter in 1:iterations) {
    ...
    for (fold in 1:folds) {
      ...
      ###########
      risk_scores <- predict(cox_model_retrained, newdata = test_data, type = "risk")
      # Compute C-index
      c_index <- concordance(cox_model_retrained, newdata = test_data)$concordance
...
external_validation_time_varying_cox <- function(train_data, test_data, shorten_visits = FALSE, n_visits=NULL) {
  ...
  return(list(
    C_Index = c_index,
    AUC_Stats = auc_summary,
    model_summary = summary(cox_model_retrained)
  ))
}
...
feature_set2 = c("CDR_SB", "GDS", "MMSE", "memory_recall1", "memory_recall2", "Digit_Span_F", "Digit_Span_B", "animal", "BNT", "age", "sex", "edu","BMI")
```
...
```{r}
time_varying_ft2_NACC_full_results1 <- do_internal_cv_time_varying_cox(NACC_full_df[, unique(c(feature_set2, index_feature))], folds = 3, iterations=10, shorten_visits = FALSE)
```

```{r}
time_varying_ft2_NACC_full_results2 <- do_internal_cv_time_varying_cox(NACC_full_df[, unique(c(feature_set2, index_feature))], folds = 3, iterations=10, shorten_visits = TRUE, n_visits=3)
```


```{r}
time_varying_ft2_NACC_full_results3 <- do_internal_cv_time_varying_cox(NACC_full_df[, unique(c(feature_set2, index_feature))], folds = 3, iterations=10, shorten_visits = TRUE, n_visits=2)
```

```{r}
time_varying_ft2_NACC_full_results4 <- do_internal_cv_time_varying_cox(NACC_full_df[, unique(c(feature_set2, index_feature))], folds = 3, iterations=10, shorten_visits = TRUE, n_visits=1)
```
```{r}
time_varying_ft2_full_external_results1 <- external_validation_time_varying_cox(NACC_full_df[, unique(c(feature_set2, index_feature))], AIBL_full_df_imputed[, unique(c(feature_set2, index_feature))], shorten_visits = FALSE)
```

```{r}
time_varying_ft2_full_external_results2 <- external_validation_time_varying_cox(NACC_full_df[, unique(c(feature_set2, index_feature))], AIBL_full_df_imputed[, unique(c(feature_set2, index_feature))], shorten_visits = TRUE, n_visits = 3)
```


```{r}
time_varying_ft2_full_external_results3 <- external_validation_time_varying_cox(NACC_full_df[, unique(c(feature_set2, index_feature))], AIBL_full_df_imputed[, unique(c(feature_set2, index_feature))], shorten_visits = TRUE, n_visits = 2)
```

```{r}
time_varying_ft2_full_external_results4 <- external_validation_time_varying_cox(NACC_full_df[, unique(c(feature_set2, index_feature))], AIBL_full_df_imputed[, unique(c(feature_set2, index_feature))], shorten_visits = TRUE, n_visits = 1)
```
...
## all Feature
```{r}
time_varying_all_NACC_full_results1 <- do_internal_cv_time_varying_cox(NACC_full_df[, unique(c(Cog_fts, demo_fts, basic_fts, index_feature))], folds = 3, iterations=10, shorten_visits = FALSE)
```

```{r}
time_varying_all_NACC_full_results2 <- do_internal_cv_time_varying_cox(NACC_full_df[, unique(c(Cog_fts, demo_fts, basic_fts, index_feature))], folds = 3, iterations=10, shorten_visits = TRUE, n_visits=3)
```

```{r}
time_varying_all_NACC_full_results3 <- do_internal_cv_time_varying_cox(NACC_full_df[, unique(c(Cog_fts, demo_fts, basic_fts, index_feature))], folds = 3, iterations=10, shorten_visits = TRUE, n_visits=2)
```

```{r}
time_varying_all_NACC_full_results4 <- do_internal_cv_time_varying_cox(NACC_full_df[, unique(c(Cog_fts, demo_fts, basic_fts, index_feature))], folds = 3, iterations=10, shorten_visits = TRUE, n_visits=1)
```


```{r}
time_varying_all_external_full_results1 <- external_validation_time_varying_cox(NACC_full_df[, unique(c(Cog_fts, demo_fts, basic_fts, index_feature))], AIBL_full_df_imputed[, unique(c(Cog_fts, demo_fts, basic_fts, index_feature))], shorten_visits = FALSE)
```

```{r}
time_varying_all_external_full_results2 <- external_validation_time_varying_cox(NACC_full_df[, unique(c(Cog_fts, demo_fts, basic_fts, index_feature))], AIBL_full_df_imputed[, unique(c(Cog_fts, demo_fts, basic_fts, index_feature))], shorten_visits = TRUE, n_visits = 3)
```

```{r}
time_varying_all_external_full_results3 <- external_validation_time_varying_cox(NACC_full_df[, unique(c(Cog_fts, demo_fts, basic_fts, index_feature))], AIBL_full_df_imputed[, unique(c(Cog_fts, demo_fts, basic_fts, index_feature))], shorten_visits = TRUE, n_visits = 2)
```

```{r}
time_varying_all_external_full_results4 <- external_validation_time_varying_cox(NACC_full_df[, unique(c(Cog_fts, demo_fts, basic_fts, index_feature))], AIBL_full_df_imputed[, unique(c(Cog_fts, demo_fts, basic_fts, index_feature))], shorten_visits = TRUE, n_visits = 1)
```
...
```{r}
plot_auc_trajectory_group(list(time_varying_all_NACC_full_results1$AUC_Stats, time_varying_all_NACC_full_results2$AUC_Stats, time_varying_all_NACC_full_results3$AUC_Stats, time_varying_all_NACC_full_results4$AUC_Stats), c("full visits", "3-visits", "2-visits", "1-visits"), title_name = "for all features on Full longitudinal data", output_file = "../results/time-varying AUC NACC.pdf")

```


```{r}
plot_auc_trajectory_single(list(time_varying_all_external_full_results1$AUC_Stats, time_varying_all_external_full_results2$AUC_Stats, time_varying_all_external_full_results3$AUC_Stats, time_varying_all_external_full_results4$AUC_Stats), c("full visits", "3-visits", "2-visits", "1-visits"), title_name = " ", output_file = "../results/time-varying AUC external.pdf")

```

```{r}
auc_plot <- ggplot(time_varying_all_NACC_full_results1$Significant_Features_Freq, aes(x = Frequency, y = reorder(Feature, Frequency))) +
...
# Print the plot (optional, to view in R)
print(auc_plot)
```
```{r}
create_forest_plot(time_varying_all_external_full_results1$model_summary, "Forest Plot for Time-Varying Cox Model", "../results/forest_plot_all_visits.pdf")
```
