  ...
  for (iter in 1:iterations) {
    ...
    for (fold in 1:folds) {
      ...
      if (!any(is.infinite(coef(cox_model)))) {
        ...
        cat("Model did not converge properly; significant features cannot be reliably determined.\n")
      }
...
ggsave("../results/feature_importance_plot_all_visits_test.pdf", plot = ft_plot, device = "pdf", width = 8, height = 6)
...
### selected Feature
```{r}
selected_fts =c("CDR_SB","memory_recall2", "animal", "APOE4", "edu", "MMSE")
...
```{r}
plot_auc_trajectory_group(list(aggregate_APOE4_NACC_results1$AUC_Stats, aggregate_APOE4_NACC_results2$AUC_Stats, aggregate_APOE4_NACC_results3$AUC_Stats), c("full visits", "3-visits", "2-visits"), title_name = " ", output_file = "../results/agg-cox AUC internal APOE4.pdf")
```
...
# static_ft = c("edu", "APOE4")
# dynamic_fts = c("CDR_SB", "MMSE","memory_recall2", "animal")
...
```{r}
plot_auc_trajectory_group(list(aggregate_all_NACC_results1$AUC_Stats, aggregate_all_NACC_results2$AUC_Stats, aggregate_all_NACC_results3$AUC_Stats), c("full visits", "3-visits", "2-visits"), title_name = " ", output_file = "../results/agg-cox AUC internal all fts.pdf")
```

### selected feature
```{r}
static_ft2 = c("edu", "APOE4")
...
```{r}
plot_auc_trajectory_group(list(aggregate_fts_NACC_results1$AUC_Stats, aggregate_fts_NACC_results2$AUC_Stats, aggregate_fts_NACC_results3$AUC_Stats), c("full visits", "3-visits", "2-visits"), title_name = " ", output_file = "../results/agg-cox AUC internal select fts.pdf")
```
...
```{r}
plot_auc_trajectory_single(list(aggregate_fts_AIBL_results1$AUC_Stats, aggregate_fts_AIBL_results2$AUC_Stats, aggregate_fts_AIBL_results3$AUC_Stats), c("full visits", "3-visits", "2-visits"), title_name = " ", output_file = "../results/aggre-cox AUC external selected fts.pdf")
```
...
```{r}
baseline_AIBL_results1 <- do_external_normal_cox(NACC_baseline_df[, unique(c(final_selected_fts, c("event", "time", "ID") ))], AIBL_baseline_df[, unique(c(final_selected_fts, c("event", "time", "ID") ))], shorten_visits = FALSE)

```

```{r}
plot_auc_trajectory_group(list(baseline_all_NACC_results$AUC_Stats, baseline_APOE4_NACC_results$AUC_Stats, baseline_fts_NACC_results$AUC_Stats), c("full features", "APOE+age", "selected features"), validation_auc_stats = baseline_AIBL_results1$AUC_Stats, validation_label = "External (selected features)", title_name = " ", output_file = "../results/baselin_cox AUC.pdf")
